...
3. **File detail view** — click any file name to see source code with
   ...
   - **Red** = never executed
   - **Yellow** (branches) = some branches taken, others missed
...
the date IS before 1930. The "happy path" (date after 1930, no error
added) is never exercised.
...
conditional. The SimpleCov HTML report shows this with yellow highlights
...
**File:** `spec/examples/false_security_spec.rb`

This file contains a small `shipping_cost` method and two specs that
achieve **100% C0 and 100% C1 coverage**:
...
1. Run this spec:
   ...
   ```
   Both tests pass. Coverage is 100%.

2. Now answer: **What happens when `weight == 10`?**
...
3. **Discussion:** Coverage measures **execution**, not **correctness**.
...
**Bonus:** Write a third spec that tests `shipping_cost(10)`. What
should the expected value be? You need the *requirements* (not just the
code) to answer this.
...
2. **Explore branch coverage.** Find a yellow-highlighted line in the
